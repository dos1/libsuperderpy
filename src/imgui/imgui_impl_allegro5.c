	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				al_set_clipping_rectangle((int)(pcmd->ClipRect.x - clip_off.x), (int)(pcmd->ClipRect.y - clip_off.y), (int)(pcmd->ClipRect.z - pcmd->ClipRect.x), (int)(pcmd->ClipRect.w - pcmd->ClipRect.y));
	...
	switch (ev->type) {
		...
		case ALLEGRO_EVENT_KEY_CHAR:
			if (ev->keyboard.display == g_Display) {
				ImGuiIO_AddInputCharacter(io, (unsigned int)ev->keyboard.unichar);
			}
			return true;
